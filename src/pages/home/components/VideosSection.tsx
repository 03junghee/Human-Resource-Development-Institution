...
// 1. 유튜브 URL에서 비디오 ID를 추출하는 유틸리티 함수 추가
// 예: https://www.youtube.com/watch?v=hJP5GqnTrNo -> hJP5GqnTrNo
...
const videos = [
  ...
  },
  {
    ...
    color: 'secondary',
    // 예시 URL들입니다. 실제 연계된 영상 URL로 나중에 변경하세요.
    url: 'https://www.youtube.com/watch?v=7b5anr8KS-Y', 
  },
  {
    id: 3,
    ...
    color: 'primary',
    url: 'https://www.youtube.com/watch?v=nsU5wbBd_iY',
  },
  {
    id: 4,
    ...
    tag: 'OECD 연계 및 정책',
    color: 'accent',
    url: 'https://www.youtube.com/watch?v=B3YwIMGBNr4',
  },
  {
    id: 5,
  ...
  return (
    <section ref={ref} className="py-20 md:py-28 px-6 md:px-10 bg-background-50">
      <div className="max-w-[1080px] mx-auto">
        <div className="mb-2">
          <span className="text-xs font-medium text-accent-600 uppercase tracking-widest">
            Research Insight
          </span>
        </div>
        <h2 className="text-xl md:text-3xl font-bold text-primary-600 mb-2">영상</h2>
        <p className="text-sm text-foreground-400 mb-12 md:mb-16">
          AI 시대의 교육, 인지, 주체성을 이해하기 위한 주요 강연
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {videos.map((video, idx) => {
            ...
            // 썸네일 이미지 URL (hqdefault는 고화질 기본 썸네일입니다)
            const thumbnailUrl = youtubeId 
              ? `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`
              : '/path/to/default/image.jpg'; // ID 추출 실패 시 보여줄 기본 이미지 주소
            ...
            return (
              ...
              >
                ...
                <a
                  ...
                  className="relative bg-background-100 h-32 sm:h-36 md:h-40 flex items-center justify-center cursor-pointer block overflow-hidden"
                >
                  {/* 유튜브 썸네일 이미지 태그 추가 */}
                  <img 
                    src={thumbnailUrl} 
                  ...
                  {/* 기존 재생 버튼 영역을 이미지 위에 겹쳐서 표시 */}
                  <div className="absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-background-50/90 flex items-center justify-center scale-90 group-hover:scale-100 transition-all duration-300">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                        tagColors[video.color] || tagColors.accent
                      }`}
                    >
                      {video.tag}
                    </span>
                  </div>
                  <h4 className="text-sm font-semibold text-foreground-800 mb-1.5 leading-snug">
                    {video.title}
                  </h4>
